...
export async function sendFriendRequest(req,res) {
    try {
        ...
        // check user already friends
        if(recipient.friends.includes(myId)){
            return res.status(400).json({ message: "You are already friends with this user"});
        }

        // check reques already exists
...
export async function acceptFriendRequest(req,res) {
    try {
        ...
        // add each user to other friend list
        // $addtoSet - add elements to set if they don't exist
...
export async function getFriendRequests(req,res) {
    try {
        ...
        console.error("Error in get friend request controller", error.message);
        res.status(500).json({ message: "Internal Server Error"});
    }
}
...
export async function getOutgoingFriendReqs(req,res) {
    try {
        ...
        console.error("Error in get outgoing friend request controller", error.message);
        res.status(500).json({ message: "Internal Server Error"});
    }
}